...
MEMORY {
  ...
  ram (wx)         : ORIGIN = DMEM_BASE, LENGTH = DMEM_SIZE-0x400
  rom (rx)         : ORIGIN = PMEM_BASE, LENGTH = PMEM_SIZE     
  srom (rx)        : ORIGIN = SMEM_BASE, LENGTH = SMEM_SIZE
  SECUREIVT (rwx)  : ORIGIN = 0x5EE0, LENGTH = 0x0020
  ERRORWORD (rwx)  : ORIGIN = 0x5F00, LENGTH = 0x0002
  SECUREVARS (rwx) : ORIGIN = 0x5F02, LENGTH = 0x0100
  GLOBALVARS (rwx) : ORIGIN = 0x6000, LENGTH = 0x0100
  SECURESTACK (rwx) : ORIGIN = 0x6100, LENGTH = 0x0100
  IPESTRUCT (rx)   : ORIGIN = 0xf000, LENGTH = 0x0008
  SECUREGW (rx)    : ORIGIN = 0xf008, LENGTH = 0x0300
  SECUREFRAM (rx)  : ORIGIN = 0xf308, LENGTH = 0x0100
...
SECTIONS
{
  ...
  .stack2 (ORIGIN (SECURESTACK) + LENGTH(SECURESTACK)) :
  {
    PROVIDE (__stack2 = .);
    *(.stack2)
  }
