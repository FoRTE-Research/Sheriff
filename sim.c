...
#define SECZONE_START       0x4000

#define SECURE_GV_ADDRESS   0x4008

#define SECGW_CALL_ADDRESS  0x4048

#define EXIT_RET_ADDRESS    0x4094

#define EXIT_IRQ_ADDRESS    0x40C0

#define IRQ_RESET_ADDRESS   0x41DE

#define ENTRY_RETI_ADDRESS  0x41E6

#define SECUREFRAM_ADDRESS  0x4248

#define SECURESTACK_ADDRESS 0xDF80

#define SECZONE_END         0xEF7F

bool IsSecureSimulator = false; 

bool IsSecZoneActive   = false;

bool IsSecureInterrupt = false;

bool IsNormalInterrupt = false;

FILE *traceFp, *heatFp;
//FILE *heatFp;
unsigned long cycleCount = 0, lastCount = 0;
unsigned long instructionFetches = 0, lastInstructionFetches = 0;
...
addrAccesses heatTracker[MEM_SIZE];
...
static int add_to_pc(struct sim_device *dev, int16_t offset);
...
static int mem_setb(struct sim_device *dev, uint32_t offset, uint8_t value)
{
	...
	/*** Check illegal write to secure zone from insecure zone ***/
	if (IsSecZoneActive == true) {
	    if ((SECZONE_START <= offset && offset <= SECZONE_END) 
	        && (dev->current_insn < SECZONE_START || dev->current_insn > SECZONE_END)) {
	            printc_err("Illegal write attempt to secure zone (0x%05x) at PC= 0x%05x!\n",offset,dev->current_insn);
			    return -1;
	    } else if ((SECGW_CALL_ADDRESS <= offset && offset < SECUREFRAM_ADDRESS)
            && (SECZONE_START <= dev->current_insn && dev->current_insn <= SECZONE_END)) {
                printc_err("Writes to secure gateway are not allowed!\n");
                return -1;
        } else if (((SECZONE_START <= offset && offset < SECURE_GV_ADDRESS)
            || (SECUREFRAM_ADDRESS <= offset && offset < SECURESTACK_ADDRESS))
            && (SECGW_CALL_ADDRESS <= dev->current_insn && dev->current_insn < SECUREFRAM_ADDRESS)) {
                printc_err("Secure gateway can only write to global variables and secure stack!\n");
                return -1;
        }
	}
	...
	mem[offset] = value;
  //fprintf(traceFp, "%lu,w,0x%04x,0x%04x,0x%04x\n", cycleCount, offset, oldWord, newWord);
  heatTracker[offset].writes += 1;  // Not sure if this would also count as a write to offset + 1
	return 0;
}
static int mem_setw(struct sim_device *dev, uint32_t offset, uint16_t value)
{
  ...
  //fprintf(traceFp, "%lu,w,0x%04x,0x%04x,0x%04x\n", cycleCount, offset, oldWord, newWord);
  heatTracker[offset].writes += 1;
  heatTracker[offset + 1].writes += 1;
	return 0;
}
static int mem_seta(struct sim_device *dev, uint32_t offset, uint32_t value)
{
...
static uint16_t mem_getw(struct sim_device *dev, uint32_t offset)
{
	...
	if (IsSecZoneActive == true) {
	    if ((SECZONE_START <= offset && offset <= SECZONE_END) 
	        && (dev->current_insn < SECZONE_START || dev->current_insn > SECZONE_END)) {
	            printc_err("Illegal read attempt from secure zone (0x%05x) at PC = 0x%05x!\n",offset,dev->current_insn);
			    return -1;
	    } else if ((EXIT_RET_ADDRESS <= offset && offset < ENTRY_RETI_ADDRESS)
	        && ((SECZONE_START <= dev->current_insn && dev->current_insn < EXIT_RET_ADDRESS)
	        || (ENTRY_RETI_ADDRESS <= dev->current_insn && dev->current_insn <= SECZONE_END))) {
	            printc_err("Exit routines can only be read by the exit routines!\n");
                return -1;
        } else if (((SECGW_CALL_ADDRESS <= offset && offset < EXIT_RET_ADDRESS)
            || (ENTRY_RETI_ADDRESS <= offset && offset < SECUREFRAM_ADDRESS))
            && ((EXIT_RET_ADDRESS <= dev->current_insn && dev->current_insn < ENTRY_RETI_ADDRESS)
            || (SECZONE_START <= dev->current_insn && dev->current_insn < SECGW_CALL_ADDRESS)
            || (SECUREFRAM_ADDRESS <= dev->current_insn && dev->current_insn <= SECZONE_END))) {
                printc_err("Entry routines can only be read by the entry routines!\n");
                return -1;
        } else if (((SECZONE_START <= offset && offset < SECURE_GV_ADDRESS)
            || (SECUREFRAM_ADDRESS <= offset && offset < SECURESTACK_ADDRESS))
            && (SECGW_CALL_ADDRESS <= dev->current_insn && dev->current_insn < SECUREFRAM_ADDRESS)) {
                printc_err("Secure gateway can only read from global variables and secure stack!\n");
                return -1;
        }
	}
		
	uint8_t *mem = dev->memory;
  //fprintf(traceFp, "%lu,r,0x%04x,0x%04x,n\n", cycleCount, offset, (mem[offset] | (mem[offset+1] << 8)));
  heatTracker[offset].reads += 1;
  heatTracker[offset + 1].reads += 1;
...
static int add_to_pc(struct sim_device *dev, int16_t offset)
{
	...
	/*** Code to check illegal PC access ***/
	if (IsSecZoneActive == true) {
        if ((SECZONE_START <= pc && pc <= SECZONE_END)
            && (dev->current_insn < SECZONE_START || dev->current_insn > SECZONE_END)) {
                printc_err("PC is not allowed to go into secure zone (0x%05x) from insecure zone (0x%05x)!\n",pc,dev->current_insn);
                return -1;
        } else if (((SECZONE_START <= pc && pc < EXIT_RET_ADDRESS) 
            || (ENTRY_RETI_ADDRESS <= pc && pc <= SECZONE_END))
            && (EXIT_RET_ADDRESS <= dev->current_insn && dev->current_insn < ENTRY_RETI_ADDRESS)) {
                printc_err("PC is not allowed to go to secure zone from certain parts of secure gateway!\n");
                return -1;
        } else if ((pc < SECZONE_START || SECZONE_END < pc)
            && ((SECZONE_START <= dev->current_insn && dev->current_insn < EXIT_RET_ADDRESS) 
            || (ENTRY_RETI_ADDRESS <= dev->current_insn && dev->current_insn <= SECZONE_END))) {
                printc_err("PC is not allowed to directly go to insecure zone from secure zone\n");
                return -1;
        } else if (((SECGW_CALL_ADDRESS <= pc && pc < EXIT_RET_ADDRESS)
            || (ENTRY_RETI_ADDRESS <= pc && pc < SECUREFRAM_ADDRESS))
            && ((SECZONE_START <= dev->current_insn && dev->current_insn < SECGW_CALL_ADDRESS)
            || (SECUREFRAM_ADDRESS <= dev->current_insn && dev->current_insn <= SECZONE_END))) {
                printc_err("PC is not allowed to go to entry routines from secure zone\n");
                return -1;
        }
    }
    /*** End of illegal access check ***/
			     ...
			     int is_write)
{
	...
	for (i = 0; i < DEVICE_MAX_BREAKPOINTS; i++) {
		...
		if ((bp->flags & DEVICE_BP_ENABLED) &&
		    ...
		    /*** Watchpoint access check ***/
		    if (IsSecZoneActive == true) {
	            if (SECZONE_START <= addr && addr <= SECZONE_END) {
                    printc_dbg("Cannot break in secure memory!\n");
                    return;   
                }
            }
            /*** Watchpoint access check ends ***/
...
static int step_double(struct sim_device *dev, uint16_t ins, uint16_t ext)
{
	...
	while (rept--) {
		...
		/* Code to check illegal PC access */
		temp_data = res_data & mask;
		//printf("temp_data = 0x%05x\n",temp_data);
		if (IsSecZoneActive == true) {
		    if (amode_dst == MSP430_AMODE_REGISTER && dreg == MSP430_REG_PC) { 
		        if ((SECZONE_START <= temp_data && temp_data <= SECZONE_END) && 
		            (dev->current_insn < SECZONE_START || dev->current_insn > SECZONE_END)) {
		                printc_err("Invalid write (0x%05x) to PC from insecure zone (0x%05x)!\n",temp_data,dev->current_insn);
			            return -1;
		        } else if (((SECZONE_START <= temp_data && temp_data < EXIT_RET_ADDRESS) 
		            || (ENTRY_RETI_ADDRESS <= temp_data && temp_data <= SECZONE_END))
	                && (EXIT_RET_ADDRESS <= dev->current_insn && dev->current_insn < ENTRY_RETI_ADDRESS)) {
	                    printc_err("PC is not allowed to go back to secure zone from certain parts of secure gateway!\n");
			            return -1;
                } else if ((temp_data < SECZONE_START || SECZONE_END < temp_data)
	                && ((SECZONE_START <= dev->current_insn && dev->current_insn < EXIT_RET_ADDRESS) 
	                || (ENTRY_RETI_ADDRESS <= dev->current_insn && dev->current_insn <= SECZONE_END))) {
	                    printc_err("PC is not allowed to go directly to insecure zone from secure zone\n");
			            return -1;
                } else if (((SECGW_CALL_ADDRESS <= temp_data && temp_data < EXIT_RET_ADDRESS)
                    || (ENTRY_RETI_ADDRESS <= temp_data && temp_data < SECUREFRAM_ADDRESS))
                    && ((SECZONE_START <= dev->current_insn && dev->current_insn < SECGW_CALL_ADDRESS)
                    || (SECUREFRAM_ADDRESS <= dev->current_insn && dev->current_insn <= SECZONE_END))) {
                        printc_err("PC is not allowed to go to entry routines from secure zone\n");
                        return -1;
...
static int step_single(struct sim_device *dev, uint16_t ins, uint16_t ext)
{
	...
	while (rept--) {
		...
		case MSP430_OP_CALL:
			...
			/* Calls from outside secure memory zone to secure memory zone are not
			 * allowed, only CALLA (CPUX) to secGW_call entry point is allowed */
			if (IsSecZoneActive == true) {
			    if ((SECZONE_START <= src_data && src_data <= SECZONE_END)
			        && (dev->current_insn < SECZONE_START || dev->current_insn > SECZONE_END)) {
			            printc_err("Invalid call to secure zone from insecure zone (0x%05x)!\n",dev->current_insn);
			            return -1;
		        } else if (((SECZONE_START <= src_data && src_data < EXIT_RET_ADDRESS) 
	                || (ENTRY_RETI_ADDRESS <= src_data && src_data <= SECZONE_END))
                    && (EXIT_RET_ADDRESS <= dev->current_insn && dev->current_insn < ENTRY_RETI_ADDRESS)) {
                        printc_err("PC is not allowed to go back to secure zone from certain parts of secure gateway!\n");
		                return -1;
                } else if ((src_data < SECZONE_START || SECZONE_END < src_data)
                    && ((SECZONE_START <= dev->current_insn && dev->current_insn < EXIT_RET_ADDRESS) 
                    || (ENTRY_RETI_ADDRESS <= dev->current_insn && dev->current_insn <= SECZONE_END))) {
                        printc_err("PC is not allowed to go directly to insecure zone from secure zone\n");
		                return -1;
                } else if (((SECGW_CALL_ADDRESS <= src_data && src_data < EXIT_RET_ADDRESS)
                    || (ENTRY_RETI_ADDRESS <= src_data && src_data < SECUREFRAM_ADDRESS))
                    && ((SECZONE_START <= dev->current_insn && dev->current_insn < SECGW_CALL_ADDRESS)
                    || (SECUREFRAM_ADDRESS <= dev->current_insn && dev->current_insn <= SECZONE_END))) {
                        printc_err("PC is not allowed to go to entry routines from secure zone\n");
                        return -1;
		...
		case MSP430_OP_RETI:
			...
			{
			/* Check if secure interrupt flag is set, if so clear it,
			 * and jump to entry_reti */
			if (IsSecZoneActive == true) {
			    if(IsSecureInterrupt == true && IsNormalInterrupt == false) {
				        IsSecureInterrupt = false;
				        dev->regs[MSP430_REG_PC] = ENTRY_RETI_ADDRESS;
				        store_results = 0;
				        cycles = 0;
				        break;
			    }
			    
			    IsNormalInterrupt = false;
			}
			
			uint16_t w1 = mem_getw(dev, dev->regs[MSP430_REG_SP]);
			uint16_t w2 = mem_getw(dev, dev->regs[MSP430_REG_SP] + 2);
			
			/* Code to check illegal PC access */
			if (IsSecZoneActive == true) {
                if ((SECZONE_START <= w2 && w2 <= SECZONE_END) && 
                    (dev->current_insn < SECZONE_START || dev->current_insn > SECZONE_END)) {
                        printc_err("Invalid write (0x%05x) to PC from insecure zone (0x%05x)!\n",w2,dev->current_insn);
                        return -1;
                } else if (((SECZONE_START <= w2 && w2 < EXIT_RET_ADDRESS) 
                    || (ENTRY_RETI_ADDRESS <= w2 && w2 <= SECZONE_END))
                    && (EXIT_RET_ADDRESS <= dev->current_insn && dev->current_insn < ENTRY_RETI_ADDRESS)) {
                        printc_err("PC is not allowed to go back to secure zone from certain parts of secure gateway!\n");
                        return -1;
                } else if ((w2 < SECZONE_START || SECZONE_END < w2)
                    && ((SECZONE_START <= dev->current_insn && dev->current_insn < EXIT_RET_ADDRESS) 
                    || (ENTRY_RETI_ADDRESS <= dev->current_insn && dev->current_insn <= SECZONE_END))) {
                        printc_err("PC is not allowed to go directly to insecure zone from secure zone\n");
                        return -1;
                } else if (((SECGW_CALL_ADDRESS <= w2 && w2 < EXIT_RET_ADDRESS)
                    || (ENTRY_RETI_ADDRESS <= w2 && w2 < SECUREFRAM_ADDRESS))
                    && ((SECZONE_START <= dev->current_insn && dev->current_insn < SECGW_CALL_ADDRESS)
                    || (SECUREFRAM_ADDRESS <= dev->current_insn && dev->current_insn <= SECZONE_END))) {
                        printc_err("PC is not allowed to go to entry routines from secure zone\n");
                        return -1;
	...
	/* Code to check illegal PC access */
	//printf("res_data = 0x%05x",res_data);
	if (IsSecZoneActive == true) {
	    if (store_results && amode == MSP430_AMODE_REGISTER && reg == MSP430_REG_PC) {
	        if ((SECZONE_START <= res_data && res_data <= SECZONE_END)
	            && (dev->current_insn < SECZONE_START || dev->current_insn > SECZONE_END)) {
	                printc_err("Invalid write (0x%05x) to PC from insecure zone (0x%05x)!\n",res_data,dev->current_insn);
		            return -1;
		    } else if (((SECZONE_START <= res_data && res_data < EXIT_RET_ADDRESS) 
	            || (ENTRY_RETI_ADDRESS <= res_data && res_data <= SECZONE_END))
                && (EXIT_RET_ADDRESS <= dev->current_insn && dev->current_insn < ENTRY_RETI_ADDRESS)) {
                    printc_err("PC is not allowed to go back to secure zone from certain parts of secure gateway!\n");
		            return -1;
            } else if ((res_data < SECZONE_START || SECZONE_END < res_data)
                && ((SECZONE_START <= dev->current_insn && dev->current_insn < EXIT_RET_ADDRESS) 
                || (ENTRY_RETI_ADDRESS <= dev->current_insn && dev->current_insn <= SECZONE_END))) {
                    printc_err("PC is not allowed to go directly to insecure zone from secure zone\n");
		            return -1;
            } else if (((SECGW_CALL_ADDRESS <= res_data && res_data < EXIT_RET_ADDRESS)
                || (ENTRY_RETI_ADDRESS <= res_data && res_data < SECUREFRAM_ADDRESS))
                && ((SECZONE_START <= dev->current_insn && dev->current_insn < SECGW_CALL_ADDRESS)
                || (SECUREFRAM_ADDRESS <= dev->current_insn && dev->current_insn <= SECZONE_END))) {
                    printc_err("PC is not allowed to go to entry routines from secure zone\n");
                    return -1;
...
static int step_pushm_popm(struct sim_device *dev, uint16_t ins)
{
    ...
    /* Modified to include trap for illegal access attempts to the secure zone 
       from insecure zone */
	case MSP430_OP_POPM:
		while (rept--) {
      if (is_aword) {
        temp_data = mem_geta(dev, dev->regs[MSP430_REG_SP]);
        
        /* Code to check illegal PC access */
	    //printf("temp_data = 0x%05x\n",temp_data);
	    if (IsSecZoneActive == true) {
	        if (reg == MSP430_REG_PC) {
	            if ((SECZONE_START <= temp_data && temp_data <= SECZONE_END)
	                && (dev->current_insn < SECZONE_START || dev->current_insn > SECZONE_END)) {
	                    printc_err("Invalid write (0x%05x) to PC from insecure zone (0x%05x)!\n",temp_data,dev->current_insn);
		                return -1;
        ...
        dev->regs[reg++] = temp_data;
        dev->regs[MSP430_REG_SP] += 4;
      }else{
              temp_data = mem_getw(dev, dev->regs[MSP430_REG_SP]);
...
static int step_reti_calla(struct sim_device *dev, uint16_t ins)
{
	...
	switch ((ins & 0x00C0)>>6) {
		...
		calla_common:
			...
			/* Calls from outside secure memory zone to secure memory zone,
			 * except CALLA (CPUX) to secGW_call entry point, are not allowed */
			 
		    //printf("Call to 0x%05x from 0x%05x\n",data,dev->regs[MSP430_REG_PC]);
		    
            if (IsSecZoneActive == true) {
                if (((SECZONE_START <= data && data < SECGW_CALL_ADDRESS) 
                    || (SECGW_CALL_ADDRESS < data && data <= SECZONE_END))
			        && (dev->current_insn < SECZONE_START || dev->current_insn > SECZONE_END)) {
			            printc_err("Invalid call to secure zone from insecure zone (0x%05x)!\n",dev->current_insn);
			            return -1;
		        } else if (((SECZONE_START <= data && data < EXIT_RET_ADDRESS) 
	                || (ENTRY_RETI_ADDRESS <= data && data <= SECZONE_END))
                    && (EXIT_RET_ADDRESS <= dev->current_insn && dev->current_insn < ENTRY_RETI_ADDRESS)) {
                        printc_err("PC is not allowed to go back to secure zone from certain parts of secure gateway!\n");
		                return -1;
                } else if ((data < SECZONE_START || SECZONE_END < data)
                    && ((SECZONE_START <= dev->current_insn && dev->current_insn < EXIT_RET_ADDRESS) 
                    || (ENTRY_RETI_ADDRESS <= dev->current_insn && dev->current_insn <= SECZONE_END))) {
                        printc_err("PC is not allowed to go directly to insecure zone from secure zone\n");
		                return -1;
                } else if (((SECGW_CALL_ADDRESS <= data && data < EXIT_RET_ADDRESS)
                    || (ENTRY_RETI_ADDRESS <= data && data < SECUREFRAM_ADDRESS))
                    && ((SECZONE_START <= dev->current_insn && dev->current_insn < SECGW_CALL_ADDRESS)
                    || (SECUREFRAM_ADDRESS <= dev->current_insn && dev->current_insn <= SECZONE_END))) {
                        printc_err("PC is not allowed to go to entry routines from secure zone\n");
                        return -1;
...
static void do_reset(struct sim_device *dev)
{
    ...
    /*** Routine to activate secure zone ***/
    if (IsSecureSimulator == true) {
        if (IsSecZoneActive == false) {
            if (mem_getw(dev, SECZONE_START) == 0xBEEF) {
                printc_dbg("Secure zone is now active!\n");
                IsSecZoneActive = true;
            }
        }
    }    
    
	/* Set PC to irqRESET inside secure gateway interrupt handler
	 * if reset occurs inside secure zone */
	if (SECZONE_START <= dev->current_insn && dev->current_insn <= SECZONE_END) {
	    simio_step(dev->regs[MSP430_REG_SR], 4);
	    memset(dev->regs, 0, sizeof(dev->regs));
	    dev->regs[MSP430_REG_PC] = IRQ_RESET_ADDRESS;
...
static int step_system(struct sim_device *dev)
{
	...
	if (irq == 15) {
		...
		/* Secure Gateway Interrupt Handler Routine Start */
		if (IsSecZoneActive == true) {
		    if ((SECZONE_START <= dev->current_insn && dev->current_insn <= SECZONE_END)) {
	                
	                /* NMIs are not returnable */
	                if(irq < 14) {
	                    IsSecureInterrupt = true;
		                IsNormalInterrupt = false;
                    }
			             
		            dev->regs[MSP430_REG_PC] = EXIT_IRQ_ADDRESS + irq * 8;
		            
		            simio_ack_interrupt(irq);
		            simio_step(status, 6);
		            return 0;
	        }	
		    /* Secure Gateway Interrupt Handler Routine End */
		    
		    IsNormalInterrupt = true;
		}
...
static void sim_destroy(device_t dev_base)
{
  ...
  for(i = 0; i < MEM_SIZE; i++){
    fprintf(heatFp, "0x%X,", i);
    fprintf(heatFp, "%u,%u\n", heatTracker[i].reads, heatTracker[i].writes);
  }
  fclose(heatFp);
	free(dev_base);
}
		       ...
		       uint8_t *mem, address_t len)
{
	...
	/*** Debugger read access check ***/
	if (IsSecZoneActive == true) {
	    if ((SECZONE_START <= addr && addr <= SECZONE_END)
	        || (SECZONE_START <= (addr + len) && (addr + len) <= SECZONE_END)
	        || (addr < SECZONE_START && (SECZONE_END < (addr + len)))) {
	            printc_err("Debugger cannot read from secure memory!\n");
	            return -1;   
        }
    }
    /*** Debugger read access check ends ***/
			...
			const uint8_t *mem, address_t len)
{
	...
	/*** Debugger write access check ***/
	if (IsSecZoneActive == true) {
	    if ((SECZONE_START <= addr && addr <= SECZONE_END)
	        || (SECZONE_START <= (addr + len) && (addr + len) <= SECZONE_END)
	        || (addr < SECZONE_START && (SECZONE_END < (addr + len)))) {
	            printc_err("Debugger cannot write to secure memory!\n");
	            return -1;   
        }
    }
    /*** Debugger write access check ends ***/
...
static int sim_ctl(device_t dev_base, device_ctl_t op)
{
	...
	case DEVICE_CTL_HALT:
	    ...
	    /*** Debugger access check ***/
	    if (IsSecZoneActive == true) {
	        if (SECZONE_START <= dev->current_insn && dev->current_insn <= SECZONE_END) {
                printc_err("Debugger cannot halt device in secure memory!\n");
                return -1;   
            }
        }
        /*** Debugger access check ends ***/
	...
	case DEVICE_CTL_STEP:
	    ...
	    if (IsSecZoneActive == true) {
	        if (SECZONE_START <= dev->current_insn && dev->current_insn <= SECZONE_END) {
                printc_err("Debugger cannot single step in secure memory!\n");
                return -1;   
            }
        }
        /*** Debugger access check ends ***/
		     ...
		     address_t addr)
{
	...
	case DEVICE_ERASE_MAIN:
	    ...
	    /*** Routine for when secure zone is active ***/
	    if (IsSecZoneActive == true) {
		    memset(dev->memory + 0x2000, 0xff, SECZONE_START - 0x2000);
		    memset(dev->memory + SECZONE_END + 1, 0xff, MEM_SIZE - SECZONE_END - 1);
		    break;
		}
		/*** Routine ends ***/
	...
	case DEVICE_ERASE_ALL:
	    ...
	    /*** Routine for when secure zone is active ***/
	    if (IsSecZoneActive == true) {
		    memset(dev->memory, 0xff, SECZONE_START);
		    memset(dev->memory + SECZONE_END + 1, 0xff, MEM_SIZE - SECZONE_END - 1);
		    break;
		}
		/*** Routine ends ***/
	...
	case DEVICE_ERASE_SEGMENT:
	    ...
	    if (IsSecZoneActive == true) {
	        if ((SECZONE_START <= addr && addr <= SECZONE_END)
	            || (SECZONE_START <= (addr + 64) && (addr + 64) <= SECZONE_END)) {
                printc_err("Debugger cannot erase secure memory!\n");
                return -1;   
            }
        }
        /*** Debugger access check ends ***/
	...
	while (count > 0) {
		...
		for (i = 0; i < dev->base.max_breakpoints; i++) {
			...
			if ((bp->flags & DEVICE_BP_ENABLED) &&
			    ...
			    if (IsSecZoneActive == true) {
	                if (SECZONE_START <= dev->current_insn && dev->current_insn <= SECZONE_END) {
                        printc_dbg("Debugger cannot break in secure memory!\n");
				...
				dev->running = 0;
                printc_dbg("Device halted, cycleCount: %lu\n", cycleCount);
                printc_dbg("Cycles since last breakpoint: %lu\n", (cycleCount - lastCount));
                printc_dbg("Instruction fetches since last breakpoint: %lu\n", (instructionFetches - lastInstructionFetches));
                lastCount = cycleCount;
                lastInstructionFetches = instructionFetches;
				return DEVICE_STATUS_HALTED;
			}
		}
		...
		if (dev->watchpoint_hit) {
		    ...
		    if (IsSecZoneActive == true) {
	            if (SECZONE_START <= dev->current_insn && dev->current_insn <= SECZONE_END) {
                    printc_dbg("Device cannot break in secure memory!\n");
                    return DEVICE_STATUS_RUNNING;   
                }
            }
            /*** Debugger access check ends ***/
		...
		if (ctrlc_check()) {
		    ...
		    if (IsSecZoneActive == true) {
	            if (SECZONE_START <= dev->current_insn && dev->current_insn <= SECZONE_END) {
                    printc_dbg("Cannot use CTRL+C during secure execution!\n");
                    return DEVICE_STATUS_RUNNING;   
                }
            }
            /*** Debugger access check ends ***/
...
static device_t sim_open(const struct device_args *args)
{
  ...
  printc_dbg("\n-----CPUX COMPATIBLE MEMORY ACCESS TRACKER-----\n");
  traceFp = fopen("simout_trace.txt", "w");
  heatFp = fopen("simout_heat.txt", "w");

  fprintf(traceFp, "cycleCount,r/w,addr,oldVal,newVal (n if read)\n");
  fprintf(heatFp,"addr,reads,writes\n");
  memset(heatTracker, 0, sizeof(heatTracker));
